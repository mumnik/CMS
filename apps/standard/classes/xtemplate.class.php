...
class XTemplate {
	...
	function XTemplate($file,  $tpldir = '', $files = null, $mainblock = 'main', $autosetup = true) {
	    global $lang, $CONF, $LC, $ln;
// $request, $xReg, $xAuth, $xCalls, $xSms, $xDetails, $xBuy, $xClc, $xHelp, $f, $xnReg;
...
// set global variables
		...
		$this->assign('GL', array('lang' => $ln, 
									'base' => $CONF['base'], 
									'module' => get_request('module',$CONF['default']), 
									'action' => get_request('action',null),
									'language' =>get_word($ln)
								)
					);
...
// preprocess some stuff
     ...
     */
	function nq_assign($name, $val = '') {
		$this->vars[$name] = no_quotes($val);
	}
	...
	function parse($bname) {
		...
		else if($this->_ignore_missing_blocks){
// ------------------------------------------------------
...
// this line would think that zeros(0) were empty - which is not true
